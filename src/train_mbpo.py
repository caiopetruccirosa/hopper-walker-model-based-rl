...
LR                        = 1e-3
LR_END                    = 1.5e-4
NON_LR_ANNEALING_EPISODES = 3000
BATCH_SIZE                = 128
...
def train(agent: MBRLv1dot5Agent, checkpoint_folder: str, history_folder: str):
    ...
    history = create_history(
        attributes=[
            ...
            ('dynamics_model_loss_vs_num_update_steps', 'Agent\'s Dynamics Model Loss', 'Number of Update Steps'),
    ...
    scheduler = optim.lr_scheduler.LinearLR(optimizer, start_factor=1, end_factor=LR_END/LR, total_iters=N_EPISODES-NON_LR_ANNEALING_EPISODES)
    ...
    while episodes_completed < N_EPISODES:
        ...
        if 'episode' in infos.keys():
            ...
            if episodes_completed >= NON_LR_ANNEALING_EPISODES//2:
                scheduler.step(episodes_completed-(NON_LR_ANNEALING_EPISODES//2))
        ...
        if total_steps//TRAIN_FREQUENCY > training_step_idx and len(buffer) >= BATCH_SIZE:
            ...
            for _ in range(EPOCH_BATCH_AMOUNT):
                ...
                loss = F.mse_loss(pred_next_states, sampled_next_states) + F.mse_loss(pred_rewards.squeeze(-1), sampled_rewards)

                optimizer.zero_grad()
                loss.backward()
                optimizer.step()

                add_to_history(history, 'dynamics_model_loss_vs_num_update_steps', loss.item())
                add_to_history(history, 'lr_vs_num_episodes', scheduler.get_last_lr()[0])
